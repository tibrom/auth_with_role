...
impl HasuraClientManager {
    fn create_hasura_client() -> HasuraClient {
        let host = CredentialsManager::get_credentials().unwrap().host().clone();
    ...
    pub async fn get_hasura_client() -> HasuraClient {
        {
...
impl<'a> HasuraClientBuilder<'a> {
    ...
    fn search_files(&mut self) -> &mut Self {
        ...
        self.filenames
            .iter()
            .filter_map(|&filename| match gql_file_service.read_query(filename) {
...
mod tests {
    ...
    fn test_graphql_loader_reads_existing_file() {
        ...
        assert!(
            ...
            "Query should contain the word 'query'"
        );
    }
    ...
    fn test_builder_loads_and_adds_queries() {
        ...
        let mut builder =
            HasuraClientBuilder::new("http://localhost".to_string(), TEST_GQL_DIR.clone());
        builder.add_filename(TEST_QUERY);
        ...
        assert!(
            result_gql_builder.is_some(),
            "Expected to find GqlBuilder, but got error"
        );
        ...
        let query_json: serde_json::Value =
            serde_json::from_str(&query).expect("Expected valid JSON from gql_builder.build()");

        let actual_query = query_json
            .get("query")
