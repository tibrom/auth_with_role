...
impl JwtClaimsService for ClaimsProvider {
    ...
    fn access_claims(&self, user: &UserWithRole) -> Result<Claims, Self::Error> {
        let x_hasura_default_role = user.allowed_roles()
            .iter()
            .find(|v| *v.is_default())
            .map(|v| Ok(v.role().clone()))
            .unwrap_or_else(|| Err(JwtError::DefaultRoleMissing))?;

        let x_hasura_allowed_roles = user.allowed_roles()
            .iter()
            .map(|v|v.role().clone())
            .collect::<Vec<_>>();
        ...
        let exp = CredentialsProvider.get_credentials()
        ...
        Ok(Claims::new(x_hasura_user_id, false, expiration, hasura_claims))
    }

    fn inner_access_claims(&self, ) -> Result<Claims, Self::Error> {
        let hasura_credentials = CredentialsProvider.get_credentials()
